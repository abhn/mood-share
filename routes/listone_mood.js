                            ...
                            if (err) {
                                ...
                                if (item) {
                                    ...
                                    if(username === friendID) {
                                        statuses.find({username: username}).sort({ $natural: -1}).limit(1).toArray(function (err, data) {
                                            if (err) {
                                                res.status(500).json({status: 'error', message: 'internal server error'});
                                            }
                                            else {
                                        ...
                                        friends.findOne({username: username, friend: friendID}, function (err, exists) {
                                            if (err) {
                                                ...
                                                if (exists) {
                                                    statuses.find({username: friendID}).sort({ $natural: -1}).limit(1).toArray(function (err, data) {
                                                        if (err) {
                                                            res.status(500).json({
                                                                status: 'error',
                                                                message: 'internal server error'
